...
\setmainfont[
  ...
  \newcommand{\lsCoverTitleFont}[1]{\sffamily\addfontfeatures{Scale=MatchUppercase}\fontsize{49pt}{16.75mm}\selectfont #1}
  ...
  \newcommand{\lsBackBodyFont}{\fontsize{11pt}{5mm}\selectfont}
...
\pgfsetlayers{lspcls_bg,main}
	...
	\begin{tikzpicture}[remember picture, overlay,bg/.style={outer sep=0}]
		...
		% Text and Graphics Layer
			...
			% Book Cover
			\node [below right = 10mm and 7.5mm of lspcls_bg1.north west, text width=130mm, align=justify] (lspcls_covertitle) {\color{\lsCoverFontColour}\lsCoverTitleFont{\title\par}}; % x = 15mm - 7.5mm ; y = 17.5mm - 7.5mm
			
			\ifnum\subtitleStrLen=0  % Is there a subtitle?
				\node [below = 11.2mm of lspcls_covertitle.south, text width=130mm] {\color{\lsCoverFontColour}\lsCoverAuthorFont{\author}}; % If not, just print the author
				\else
				\node [below = 8mm of lspcls_covertitle.south, text width=130mm, align=left] (lspcls_coversubtitle) {\color{\lsCoverFontColour} \lsCoverSubTitleFont \subtitle\par}; 
				\node [below = 11.2mm of lspcls_coversubtitle.south, text width=130mm] {\color{\lsCoverFontColour}\lsCoverAuthorFont{\author}};
			\fi
