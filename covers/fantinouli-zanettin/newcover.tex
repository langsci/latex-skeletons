...
\pgfsetlayers{lspcls_bg,main}
	...
	\begin{tikzpicture}[remember picture, overlay,bg/.style={outer sep=0}]
		...
		% Text and Graphics Layer
			...
			\node [above left = 15mm and 4mm of lspcls_splinelogo.north, rotate=270] (lspcls_splinetitle) {\color{\lsSeriesColor} \lsSpineAuthorFont{\author~(eds.)} \hspace{13mm} \lsSpineTitleFont{\title}}; 
			...
			\ifnum\subtitleStrLen=0  % Is there a subtitle?
				\node [below = 11.2mm of lspcls_covertitle.south, text width=130mm] {\color{\lsCoverFontColour}\lsCoverAuthorFont{Edited~by~\author\par}}; % If not, just print the author
				\else
